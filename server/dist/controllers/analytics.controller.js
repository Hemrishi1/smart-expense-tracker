...
const getDashboardSummary = async (req, res, next) => {
    try {
        ...
        const incomeAgg = await Income_1.default.aggregate([
        ...
        const recentExpenses = await Expense_1.default.find({ user: userId }).sort({ date: -1 }).limit(10).lean();
        const recentIncomes = await Income_1.default.find({ user: userId }).sort({ date: -1 }).limit(10).lean();
        let allTransactions = [
